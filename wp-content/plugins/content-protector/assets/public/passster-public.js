jQuery(document).ready(function( $ ) {
    ...
    $('.passster-submit').on('click', function(e){
        ...
        $.ajax({
            ...
            data: { 'action': 'validate_input','nonce' : ps_ajax.nonce, 'input' : input, 'password' : password, 'post_id' : ps_ajax.post_id, 'type' : type, 'list' : list, 'partly' : partly, 'protection' : protection, 'elementor_content' : $(this).attr('data-elementor-content'), 'widget' : widget, 'acf' : acf },
            beforeSend: function() {
        ...
        grecaptcha.ready(function() {
            grecaptcha.render('ps-recaptcha-v2', {
                ...
                'callback' : function( token ) {
                    ...
                    widget     = $( '.recaptcha-v2-submit' ).attr('data-widget');
                    ...
                    $.ajax({
                        ...
                        dataType: "json",
                        url: ps_ajax.ajax_url,
                        data: { 'action': 'validate_input','nonce' : ps_ajax.nonce, 'token' : token, 'post_id' : ps_ajax.post_id, 'type' : 'recaptcha', 'protection' : protection, 'elementor_content' : $('.recaptcha-v2-submit').attr('data-elementor-content'), 'captcha_id' : ps_id, 'widget' : widget, 'acf' : acf },
                        success: function(response){
    ...
    $('.recaptcha-form').submit(function(event) {
        ...
        widget     = $(this).find('.passster-submit-recaptcha').attr('data-widget');
        ...
        grecaptcha.ready(function() {
            grecaptcha.execute(ps_ajax.recaptcha_key, {action: 'validate_input'}).then(function(token) {
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    url: ps_ajax.ajax_url,
                    data: { 'action': 'validate_input','nonce' : ps_ajax.nonce, 'token' : token, 'post_id' : ps_ajax.post_id, 'type' : 'recaptcha', 'protection' : protection,  'elementor_content' : form.find('.passster-submit-recaptcha').attr('data-elementor-content'), 'captcha_id' : ps_id, 'widget' : widget, 'acf' : acf },
                    success: function(response){
                        // todo: set cookie if activated.
                        if ( true === response.success ) {
                            // if no ajax.
        ...
        var captcha = new jCaptcha({
            ...
            callback: function(response ) {
                if (response == 'success') {
                    ...
                    widget     = $('.passster-submit-captcha').attr('data-widget');
                    ...
                    $.ajax({
                        ...
                        dataType: "json",
                        url: ps_ajax.ajax_url,
                        data: { 'action': 'validate_input','nonce' : ps_ajax.nonce, 'captcha' : 'success', 'post_id' : ps_ajax.post_id, 'type' : 'captcha', 'protection' : protection, 'elementor_content' : $('.passster-submit-captcha').attr('data-elementor-content'), 'captcha_id' : ps_id, 'widget' : widget, 'acf' : acf },
                        success: function(response){
                            if ( true === response.success ) {
                                // if no ajax.
                                if ( 'on' === ps_ajax.no_ajax ) {
