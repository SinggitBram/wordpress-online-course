...
class PS_Admin
{
    ...
    public static function init()
    {
        ...
        add_action( 'in_widget_form', array( __CLASS__, 'show_widget_id' ) );
        ...
        // Activate reload and VC when WPBakery is installed.
        
        if ( in_array( 'js_composer/js_composer.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) ) {
            $general_settings['toggle_cookie'] = 'on';
            $general_settings['toggle_ajax'] = 'on';
            $general_settings['toggle_vc'] = 'on';
            update_option( 'passster_general_settings', $general_settings );
        }
        
        // Activate reload and Elementor when Elementor is installed.
        
        if ( did_action( 'elementor/loaded' ) ) {
            $general_settings['toggle_cookie'] = 'on';
            $general_settings['toggle_ajax'] = 'on';
            $general_settings['toggle_elementor'] = 'on';
            update_option( 'passster_general_settings', $general_settings );
        }
        
        // Activate reload when Beaver Builder or Divi is installed.
        $theme = wp_get_theme();
        
        if ( in_array( 'beaver-builder-lite-version/fl-builder.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || in_array( 'bb-plugin/fl-builder.php', apply_filters( 'active_plugins', get_option( 'active_plugins' ) ) ) || 'Divi' == $theme->name || 'Divi' == $theme->parent_theme ) {
            $general_settings['toggle_cookie'] = 'on';
            $general_settings['toggle_ajax'] = 'on';
            update_option( 'passster_general_settings', $general_settings );
        }
    ...
    /**
     * Show passster widget ID in widgets.
     *
     * @param [type] $instance current widget instance.
     * @return void
     */
    public function show_widget_id( $instance )
    {
        $widget = explode( '-', $instance->id );
        echo  '<p><strong>Passster Widget ID: </strong>' . esc_html( $widget[1] ) . '</p>' ;
    }
    
    /**
     * Decrease remaining downloads in post meta.
     */
    public static function reset_usage_data()
    {
    }
