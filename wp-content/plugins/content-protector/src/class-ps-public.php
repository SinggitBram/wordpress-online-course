...
class PS_Public
{
    ...
    public function validate_input()
    {
        ...
        // Elementor content?
        $elementor_content = $_POST['elementor_content'];
        // prepare content.
        ...
        if ( !empty($_POST['widget']) ) {
            $widget = esc_html( $_POST['widget'] );
            $content = apply_filters( 'passster_ajax_widget_type', get_option( 'widget_text' ) );
            $content = $content[$widget]['text'];
        }
                ...
                if ( isset( $input ) && !empty($input) ) {
                    ...
                    if ( $hash->checkPassword( $input, $password ) === true ) {
                        ...
                        if ( 'full' !== $protection ) {
                            ...
                            // If Elementor content available.
                            if ( isset( $elementor_content ) && !empty($elementor_content) ) {
                                $content = base64_decode( $elementor_content );
                            }
    ...
    public function render_shortcode( $atts, $content = null )
    {
        ...
        if ( isset( $atts['elementor-protected-content'] ) ) {
            $form = str_replace( '[PASSSTER_ELEMENTOR_CONTENT]', base64_encode( $atts['elementor-protected-content'] ), $form );
        } else {
            $form = str_replace( '[PASSSTER_ELEMENTOR_CONTENT]', '', $form );
        }
        ...
        if ( isset( $atts['widget'] ) && true == $atts['widget'] ) {
            $form = str_replace( '[PASSSTER_WIDGET]', ' data-widget="' . $atts['widget'] . '"', $form );
        } else {
            $form = str_replace( '[PASSSTER_WIDGET]', '', $form );
        }
    ...
    {
        $global_protection_id = get_option( 'passster_global_id' );
        
        if ( !empty($global_protection_id) ) {
            $global_protection_active = get_post_meta( $global_protection_id, 'passster_activate_global_protection', true );
            
            if ( true == $global_protection_active ) {
                if ( is_page( $global_protection_id ) || is_single( $global_protection_id ) ) {
                    return;
                }
                // Check if cookie is set.
                
                if ( empty($_COOKIE['passster']) ) {
                    $global_protection_url = get_permalink( $global_protection_id );
